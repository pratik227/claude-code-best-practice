...
::: tip Example
`weather-agent` -- autonomously fetches weather data using its preloaded `weather-fetcher` skill, runs in a separate context with restricted tools.
:::
...
::: tip Example
`weather-orchestrator` -- the user triggers it, it asks for C/F preference, invokes the agent, then invokes the SVG skill.
:::
...
::: tip Example
`weather-svg-creator` -- Claude auto-invokes it when the user asks for a weather card; also callable from commands.
:::

## The Command to Agent to Skill Architecture
...
This repository has all three mechanisms defined for the same task -- displaying the current time in PKT. Here is what happens when a user types **"What is the current time?"** without explicitly invoking any `/` command:
